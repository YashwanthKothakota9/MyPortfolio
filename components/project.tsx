...
const Project = ({ title, description, tags, imageUrl }: ProjectProps) => {
  ...
  return (
    ...
    >
      <section className=" bg-teal-100 max-w-[42rem] border border-black/5 overflow-hidden sm:pr-8 relative sm:h-[20rem] rounded-lg  hover:bg-teal-200 transition sm:group-even:pl-8 dark:bg-cyan-100/10 dark:hover:bg-cyan-100/20">
        <div className="pt-4 pb-8 px-5 sm:pl-10 sm:pr-2 sm:pt-10 sm:max-w-[50%] flex flex-col h-full sm:group-even:ml-[18rem]">
          ...
          <ul className="flex flex-wrap mt-4 gap-2 sm:mt-auto">
            {tags.map((tag, index) => (
              <li
                className="bg-black/[0.7] px-3 py-1 text-[0.7rem] uppercase tracking-wider text-white rounded-full dark:text-sky-100/70"
                key={index}
              >
                {tag}
              </li>
